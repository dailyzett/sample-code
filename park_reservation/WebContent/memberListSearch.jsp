...
<div class="center">
    ...
    <form action="memberListSearch.do" method="post">
			...
			<td colspan="6" align="center">
				<c:if test="${requestScope.count gt 0 }">
					...
					<c:choose>
						...
						<c:otherwise>
							<a href="memberListSearch.do?search=<%=search %>&pageNum=${i }" class="pagenum">${i }</a>
						</c:otherwise>
					</c:choose>
					</c:forEach>
					<c:if test="${pageScope.endPage lt pageScope.pageCount }">
						<a href="memberListSearch.do?search=<%=search %>&pageNum=<%=startPage + 10 %>" class="beforenext">다음</a>
					</c:if>
				</c:if>
			</td>
		</tr>
