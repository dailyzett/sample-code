...
class PlacePaymentServiceTest : BehaviorSpec() {
    ...
    @Autowired
    lateinit var eventJdbcRepository: EventJdbcRepository
    ...
    init {
        Given("결제 요청이 주어질 때") {
            ...
            When("결제 주문 서비스가 호출되면") {
                ...
                placePaymentService.placePaymentEvent(dto)
        ...
        Given("결제 취소 요청이 주어질 때") {
            ...
            every { eventIdService.createPaymentEventId(any()) } returns cancelEventId
            paymentEventRepository.save(paymentEvent)

            When("취소 요청이 들어왔을 때") {
                placePaymentService.placePaymentCancelEvent(dto)
