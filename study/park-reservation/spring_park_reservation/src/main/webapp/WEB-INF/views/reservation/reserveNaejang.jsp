...
<body>
	...
	<div id="content">
		<div id="tab">
			<ul class="tab_menu">
				<li>
					<a href="reservation?pn=kaya">가야산</a>
				</li>
				<li>
					<a href="reservation?pn=kelong">계룡산</a>
				</li>
				<li class="on">
					<a href="reservation?pn=naejang">내장산</a>
				</li>
				<li>
					<a href="reservation?pn=seolak">설악산</a>
				</li>
			</ul>
		</div>
			...
			<table class="">
				<thead class="">
					...
					<tr>
						...
						<c:forEach var="kaya" items="${dtos }" varStatus="status">
							<td>
								<c:out value="${kaya.max_people }" />
							</td>
							<td id="date${status.index}" style="display: none;">
								<c:out value="${kaya.reservation_date }" />
							</td>
							<td id="price${status.index }" style="display: none;">
								<c:out value="${kaya.park_price }" />
							</td>
						</c:forEach>
					</tr>
					<tr>
						...
						<c:forEach var="kaya" items="${dtos }">
							...
							<c:choose>
								<c:when test="${kaya.enable_people ne kaya.max_people && kaya.enable_people ne '0' }">
									<td style="color: white; font-weight: bold; background-color: green;">
										<c:out value="${kaya.enable_people }" />
									</td>
								</c:when>

								<c:when test="${kaya.enable_people eq kaya.max_people}">
									<td style="color: white; font-weight: bold; background-color: black;">
										<c:out value="${kaya.enable_people }" />
									</td>
								</c:when>

								<c:when test="${kaya.enable_people  eq '0' }">
				...
				<tbody>
					<tr>
						...
						<c:forEach var="kaya" items="${dtos }" varStatus="status">
							<c:if test="${kaya.enable_people  eq '0' }">
								<td>
									<button type="button" id="btn${status.index }" class="reservationBtn" disabled="disabled">
										<img src="https://user-images.githubusercontent.com/73692337/107111263-f252ee80-6891-11eb-9402-0318e97c82d8.png" style="width: 30px; height: 30px;" />
									</button>
								</td>
							</c:if>
							<c:if test="${kaya.enable_people ne '0' }">
								<td>
									<button type="button" id="btn${status.index }" class="reservationBtn">
										<img src="https://user-images.githubusercontent.com/73692337/106408161-36ed1d00-6481-11eb-94bd-7edacf57357d.png" style="width: 30px; height: 30px;" />
									</button>
								</td>
							</c:if>
						</c:forEach>
	...
	<div class="detail">
		<table class="detailTable">
			...
			<tbody>
				<tr>
					...
					<td class="select_peo">
						<input type="radio" id="btn01" name="check_per" value="1" checked="">
						<label for="btn01">1명</label>
						<input type="radio" id="btn02" name="check_per" value="2">
						<label for="btn02">2명</label>
						<input type="radio" id="btn03" name="check_per" value="3">
						<label for="btn03">3명</label>
						<input type="radio" id="btn04" name="check_per" value="4">
						<label for="btn04">4명</label>
						<input type="radio" id="btn05" name="check_per" value="5">
						<label for="btn05">5명</label>
					</td>
					<td class="end cash" style="text-align: center;">
						<span class="cash_num" id="price">0</span>
						원
					</td>
				</tr>
			</tbody>
		...
		<div style="margin-top: 30px; margin-right: 80px; float: right;">
			<form action="reservation?pn=naejang" method="post" id="reservationForm">
