...
<body>
	...
	<div class="sidenav">
		...
		<a href="history" class="select">예약 현황</a>
		...
		<form action="cancel" method="post">
			<table class="qblist">
				...
				<c:forEach items="${mrdtos }" var="m">
					...
					<tr>
						<td style="width: 7%;">${m.r_id }</td>
						<c:if test="${m.park_name eq 'kaya' }">
							<c:set var="view_park" value="가야산" />
						</c:if>
						<c:if test="${m.park_name eq 'naejang' }">
							<c:set var="view_park" value="내장산" />
						</c:if>
						<c:if test="${m.park_name eq 'kelong' }">
							<c:set var="view_park" value="계룡산" />
						</c:if>
						<c:if test="${m.park_name eq 'seolak' }">
							<c:set var="view_park" value="설악산" />
						</c:if>
						<td>${view_park }</td>
						<td>${m.reservation_date}</td>
					...
					<tr style="visibility: hidden">
						<td style="visibility: hidden">
							<input type="hidden" value="${m.r_id }" name="rid">
							<input type="hidden" value="${m.reservation_date }" name="reservation_date">
							<input type="hidden" value="${m.count }" name="reservation_count">
						</td>
					</tr>
					...
					<td colspan="6" align="center">
						<c:if test="${requestScope.count gt 0 }">
							...
							<c:if test="${pageScope.startPage gt pageScope.pageBlock }">
								<a href="history?pageNum=<%=startPage - 10%>" class="beforenext">이전</a>
							</c:if>
							...
							<c:if test="${not loog_flag }">
								...
								${i }
									...
									<c:if test="${i ne currentPage }">
										<a href="history?pageNum=${i }" class="pagenum">${i }</a>
									</c:if>

								</c:forEach>
							</c:if>
							<c:if test="${pageScope.endPage lt pageScope.pageCount }">
								<a href="history?pageNum=<%=startPage + 10%>" class="beforenext">다음</a>
							</c:if>
